// Express app (shared by the local server and the Vercel serverless entrypoint).
require('./src/loadenv');
...
// Make sure the schema + settings cache are ready before handling any request.
// ensureInit() is cached, so this is a no-op after the first call (incl. serverless cold starts).
app.use(async (req, res, next) => {
  try { await db.ensureInit(); next(); } catch (e) { next(e); }
});
